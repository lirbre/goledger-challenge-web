...
interface EditCarProps {
  title?: string;
  handleClick(newKey: string, docType: string): void;
  buttonName: string;
  docType: string;
  oldItem: SearchProps;
}
...
export const EditCar = ({
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if ((newValue?.model || "").split("").length === 0) {
      toast.warn("You should choose a name to it.");
      return;
    }

    if (
      (newValue?.model === oldItem?.model) &&
      (selectedKey === oldItem?.driver?.["@key"])
    ) {
      toast.warn("You should change the name or the driver.");
      return;
    }

    handleClick(newValue?.model || "", selectedKey);
  };
