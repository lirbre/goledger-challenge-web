...
import { ControlDate } from "@/helper/controlDate";
...
export const EditEvent = ({
  ...
  const [formInfo, setFormInfo] = useState(() => {
    const parsedDate: Date = new Date(oldItem?.date || "");
    const { controledDay, controledMonth, controledYear } = ControlDate(parsedDate)

    return {
      name: oldItem?.name || "",
      prize: oldItem?.prize || 0,
      date: `${controledYear || 2022}-${controledMonth}-${controledDay}`,
      key: oldItem?.winner?.["@key"] || "",
    };
  });
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    handleClick(formInfo.name, formInfo.key, formInfo.prize, formInfo.date);
  };
